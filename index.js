...
app.post("/partida", (req, res) => {
  try {
    ...
    const partidaCriada = fs.writeFileSync(
      "./database/db.json",
      JSON.stringify(db),
      (err) => {
        if (err) {
...
app.get("/dados/partida/:id", (req, res) => {
  ...
  const partida = JSON.parse(data).find(partida => partida.id === id);
...
app.post("/excluir/:id", (req, res) => {
  ...
  try {
    ...
    const partidaExcluida = objetoPartidas.filter(
      (partida) => partida.id !== id
    );

    const novoArrayPartidas = fs.writeFileSync(
      "./database/db.json",
      JSON.stringify(partidaExcluida),
      (err) => {
        if (err) {
...
app.post("/partida/:id/adicionarJogador", (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send("Ocorreu um erro ao adicionar o jogador à partida");
  }
});
...
app.put(
  ...
  (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .send("Ocorreu um erro ao atualizar a presença do jogador");
    }
  }
);
